...
public class FilmLikedUserRepository extends BaseRepository<FilmLikedUser> {
    ...
    public static final String FIND_BY_USER_ID_QUERY = "SELECT * FROM liked_film_user WHERE user_id = ?";

    public FilmLikedUserRepository(JdbcTemplate jdbc, RowMapper<FilmLikedUser> mapper) {
        super(jdbc, mapper);
    }

    public List<FilmLikedUser> findByFilmId(long filmId) {
        return findMany(FIND_BY_ID_QUERY, filmId);
    }

    public List<FilmLikedUser> findByUserId(long userId) {
